...
from .utils import ALL_STAGES, STAGE_LABELS
...
class TripmaProductionController(TripmaBaseController):
    ...
    def production_dashboard(self, **kw):
        ...
        in_progress_orders = Order.search(
            [
                ...
                (
                    ...
                    "in",
                    ["waiting", "cutting", "printing", "assembly"],
                ),
            ]
        )
        ...
        return self._render_tripma(
            "Tripma-Sign.production_dashboard",
            {
    ...
    @http.route(
        "/tripma/production/update/submit",
        auth="user",
        methods=["POST"],
        website=True,
        csrf=True,
