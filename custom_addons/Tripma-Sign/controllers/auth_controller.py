...
class TripmaAuthController(TripmaBaseController):
    ...
    # FR-04: ROOT & WEB ROUTES — Handle default URLs
    @http.route('/', type='http', auth='public', website=True)
    def root_redirect(self, **kw):
        """
        Root URL handler — redirect based on authentication status:
        - If authenticated: redirect to role-based dashboard
        - If not authenticated: redirect to Tripma login page
        """
        if request.session.uid:
            return self._redirect_by_role()
