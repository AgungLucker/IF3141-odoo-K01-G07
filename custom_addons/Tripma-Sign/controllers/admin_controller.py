...
class TripmaAdminController(TripmaBaseController):
    ...
    def admin_dashboard(self, **kw):
        ...
        values = {
            ...
            'recent_activities': Order.search([('source_channel', 'in', ['whatsapp', 'offline', 'phone'])], order='create_date desc', limit=3)
        }
    ...
    def admin_semua_pesanan(self, **kw):
        ...
        return request.render('Tripma-Sign.admin_all_orders', values)
    ...
    def admin_export_pesanan(self, **kw):
        ...
        # Header
        writer.writerow(['No. Order', 'Tanggal', 'Pelanggan', 'Produk', 'Channel', 'Status', 'Total'])
        
        # Rows
        for order in orders:
            product_str = order.product_specs.split('\n')[0] if order.product_specs and '\n' in order.product_specs else (order.product_specs or '-')
            writer.writerow([
                order.name,
        ...
        response = request.make_response(output.getvalue(), headers=[
            ...
            ('Content-Disposition', 'attachment; filename="Semua_Pesanan_Tripma.csv"'),
        ])
        return response